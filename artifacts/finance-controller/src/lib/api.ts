...
export type AuditTrace = {
  ...
  agent_steps?: Array<unknown>;
  agent_tool_calls?: Array<unknown>;
  [key: string]: unknown;
};
...
export const getApiUrl = () =>
  localStorage.getItem('finance-api-url') || initial;

export const setApiUrl = (url: string) =>
  localStorage.setItem('finance-api-url', url.trim().replace(/\/$/, ''));
...
export const api = {
  ...
  csv: (name: string) =>
    `${getApiUrl()}/data/${encodeURIComponent(name)}.csv`,
  auditUrl: (id: string) =>
    `${getApiUrl()}/audit/${encodeURIComponent(id)}`,
};
